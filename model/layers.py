...
class PatchTransformerEncoder(nn.Module):
    def __init__(self, in_channels, patch_size=10, embedding_dim=128, num_heads=4):
        ...
        encoder_layers = nn.TransformerEncoderLayer(embedding_dim, num_heads, dim_feedforward=1024).to(device)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_layers=4).to(device)  # takes shape S,N,E

        self.embedding_convPxP = nn.Conv2d(in_channels, embedding_dim,
                                           kernel_size=patch_size, stride=patch_size, padding=0).to(device)

        self.positional_encodings = nn.Parameter(torch.rand(500, embedding_dim), requires_grad=True).to(device)
