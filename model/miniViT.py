...
class mViT(nn.Module):
    def __init__(self, in_channels, n_query_channels=128, patch_size=16, dim_out=256,
        ...
        self.conv3x3 = nn.Conv2d(in_channels, embedding_dim, kernel_size=3, stride=1, padding=1).to(device)
        self.regressor = nn.Sequential(nn.Linear(embedding_dim, 256).to(device),
                                       nn.LeakyReLU().to(device),
                                       nn.Linear(256, 256).to(device),
                                       nn.LeakyReLU().to(device),
                                       nn.Linear(256, dim_out).to(device))
